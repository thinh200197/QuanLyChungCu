...
create proc PR_INSERT_Service
	@MALOAIDICHVU INT , @TENDICHVU NVARCHAR(50) , @DONVITINH nvarchar(50) , @CREATE_BY INT 
AS
BEGIN
...
inner join DICHVU dv on dvdk.MADICHVU = dv.ID
inner join CANHO c on dvdk.MACANHO = c.MACANHO
GO

CREATE PROC PR_INSERT_DVDK 
	@MADICHVU INT,@MACANHO VARCHAR(30), @TRANG_THAI NVARCHAR(30), @STAFF INT
AS
BEGIN
	INSERT DANGKY_DINHKY (MADICHVU , MACANHO,TRANG_THAI,CREATE_BY , CREATE_DATE)
	VALUES (@MADICHVU , @MACANHO,@TRANG_THAI,@STAFF,GETDATE())
END
GO

CREATE PROC PR_UPDATE_DVDK 
	@ID INT ,@MADICHVU INT,@MACANHO VARCHAR(30), @TRANG_THAI NVARCHAR(30), @STAFF INT
AS
BEGIN
	UPDATE DANGKY_DINHKY
	SET MADICHVU = @MADICHVU , MACANHO = @MACANHO , TRANG_THAI = @TRANG_THAI , UPDATE_BY = @STAFF , UPDATE_DATE = GETDATE()
	WHERE ID = @ID
END
GO
...
AS
BEGIN
	INSERT DANGKY_CONGCONG (MADICHVU , MACUDAN,TRANG_THAI,CREATE_BY , CREATE_DATE,DONVITINH,SOLUONG)
...
BEGIN
	...
	SET MADICHVU = @MADICHVU , MACUDAN = @MACUDAN , TRANG_THAI = @TRANG_THAI , UPDATE_BY = @STAFF , UPDATE_DATE = GETDATE(),DONVITINH = @DONVITINH ,SOLUONG = @SOLUONG 
	WHERE ID = @ID
END
GO
